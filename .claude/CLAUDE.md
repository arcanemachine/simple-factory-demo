...
test/support/
  ...
  factories.ex                # Example factories (User implemented, Author commented out)
```
...
2. **Factory Inheritance** (`test/support/factories.ex`):
...
**Usage:** `FactoryManDemo.Factories.build_user(%{username: "alice"})`
...
- **Testing factories: Use :user factory (FactoryManDemo.Factories.build_user/1) - it's the root factory that others depend on**
