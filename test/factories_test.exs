defmodule FactoriesTest do
  ...
  alias FactoryManDemo.Factories.{Users, Authors, Posts, Tags}
  ...
  test "Users.build_user/0 creates a user struct" do
    ...
    assert %FactoryManDemo.Users.User{} = user
  ...
  test "Users.insert_user!/0 persists a user" do
    ...
    assert user.id
    assert Repo.get(FactoryManDemo.Users.User, user.id)
  end

  test "Authors.build_author/0 creates an author with a user" do
    author = Authors.build_author()
    assert %FactoryManDemo.Authors.Author{} = author
    assert author.name
    assert %FactoryManDemo.Users.User{} = author.user
  end
  ...
  test "Authors.insert_author!/0 persists an author and user" do
    ...
    assert Repo.get(FactoryManDemo.Authors.Author, author.id)
  end

  test "Posts.build_post/0 creates a post with author and user" do
    post = Posts.build_post()
    assert %FactoryManDemo.Posts.Post{} = post
    assert post.title
    assert post.content
    assert %FactoryManDemo.Authors.Author{} = post.author
  end
  ...
  test "Posts.insert_post!/0 persists a post with nested author and user" do
    ...
    assert Repo.get(FactoryManDemo.Posts.Post, post.id)
  end

  test "Tags.build_tag/0 creates a tag struct" do
    tag = Tags.build_tag()
    assert %FactoryManDemo.Tags.Tag{} = tag
    assert tag.name
  end
  ...
  test "Tags.insert_tag!/0 persists a tag" do
    tag = Tags.insert_tag!()
    assert tag.id
    assert Repo.get(FactoryManDemo.Tags.Tag, tag.id)
  end
  ...
  test "can create post with custom tags" do
    ...
    assert post.id
    loaded_post = Repo.get(FactoryManDemo.Posts.Post, post.id) |> Repo.preload(:tags)
  ...
  test "can create full nested structure" do
    ...
    loaded_post = Repo.get(FactoryManDemo.Posts.Post, post.id) |> Repo.preload([:tags, author: :user])
